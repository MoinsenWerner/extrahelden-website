...
$expectedToken = getenv('REC_SYNC_TOKEN') ?: 'change-me';
...
require_auth($expectedToken);

$method = $_SERVER['REQUEST_METHOD'] ?? 'GET';
$path = request_path();

if ($method === 'POST' && $path === '/api/recordings/start') {
    $data = request_json();
...
if ($method === 'POST' && $path === '/api/recordings/finish') {
    $data = request_json();
