...
/* ---------- Oversize-Guard (muss vor CSRF-Prüfung passieren!) ---------- */
...
if (!function_exists('fmt_bytes')) {
    function fmt_bytes(int $b): string {
        $units = ['B','KB','MB','GB','TB']; $i = 0; $n = $b;
        while ($n >= 1024 && $i < count($units)-1) { $n = (int)round($n/1024); $i++; }
        return $n.' '.$units[$i];
    }
}
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($contentLen > 0 && empty($_POST) && empty($_FILES)) {
        $maxPost   = ini_bytes(ini_get('post_max_size'));
        $maxUpload = ini_bytes(ini_get('upload_max_filesize'));
        $hard      = min($maxPost ?: PHP_INT_MAX, $maxUpload ?: PHP_INT_MAX);
        $msg = 'Upload zu groß. Maximale Größe: '.fmt_bytes($hard)
...
function ensure_schema(): void {
    ...
    // Sicherheitsnetz: documents.is_public (falls db.php älter war)
    $dCols  = $pdo->query('PRAGMA table_info(documents)')->fetchAll();
    $dNames = array_map(fn($r)=>$r['name'], $dCols);
    if (!in_array('is_public', $dNames, true)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf']) || !hash_equals($_SESSION['csrf'] ?? '', $_POST['csrf'])) {
        flash('Ungültiges CSRF-Token.','error'); header('Location: admin.php'); exit;
    }
    $a = $_POST['action'] ?? '';
    try {
        // Nutzer anlegen
            ...
            if (!isset($_FILES['documents'])) {
                ...
                for ($i=0; $i<$count; $i++) {
                    ...
                    if (move_uploaded_file($files['tmp_name'][$i], $target)) {
                        ...
                        trigger_auto_task('doc_uploaded', [
    'filename' => $filename,
    'scope' => ($is_public ? 'Öffentlich' : 'Privat')
]);
                        $okCnt++;
                    }
                }
        ...
        // Posts / Server / Settings
            ...
            if ($title===''||$content==='') {
                ...
                trigger_auto_task('post_created', ['content' => $content,'title' => $title]);
            }
            ...
            if ($row){ $new=((int)$row['published']===1)?0:1;
                db()->prepare('UPDATE posts SET published=? WHERE id=?')->execute([$new,$id]);
                flash($new?'Post veröffentlicht.':'Post unveröffentlicht.','success');
                $r=db()->prepare('SELECT published FROM posts WHERE id=?'); $r->execute([$id]); $row=$r->fetch();
                if ($row ==1){
                    trigger_auto_task('post_created', ['content' => $content,'title' => $title]);
                }
            ...
            else { db()->prepare('INSERT INTO minecraft_servers (name,host,port,enabled,sort_order) VALUES (?,?,?,?,?)')->execute([$name,$host,$port,$enabled,$sort]); flash('Server hinzugefügt.','success'); trigger_auto_task('server_published'); }
            ...
            // Aktuellen Status vor dem Speichern prüfen für Vergleich
            $wasEnabled = (get_setting('apply_enabled', '0') === '1');
            $nowEnabled = isset($_POST['apply_enabled']);
            
            set_setting('apply_enabled', $nowEnabled ? '1' : '0');
            ...
            // Discord Status-Update bei Änderung
            if ($wasEnabled !== $nowEnabled) {
                $targetChannel = '1477945366417641646'; // Festgelegter Kanal
                $projectName = $title;
                
                if ($nowEnabled) {
                    $msgTemplate = get_setting('msg_apply_opened', '@everyone Die Bewerbungen für {Projektname} sind eröffnet. Ihr könnt euch unter https://www.extrahelden.de/apply.php jetzt Bewerben.');
                } else {
                    $msgTemplate = get_setting('msg_apply_closed', '@everyone Die Bewerbungen für {Projektname} sind geschlossen. Viel Spaß bei der aktuellen Season.');
                }
                
                $finalMsg = str_replace('{Projektname}', $projectName, $msgTemplate);
                
                $cfg = discord_cfg();
                if ($cfg['token'] !== '') {
                    http_json('POST', "https://discord.com/api/v10/channels/{$targetChannel}/messages",
                        ['Authorization: Bot '.$cfg['token'], 'Content-Type: application/json'],
                        ['content' => $finalMsg]
                    );
                }
...
$cfg_whitelist = get_setting('whitelist_json_path', '/home/crafty/crafty-4/ervers/8c66e586-dbda-4c99-a447-b944b8677c88/whitelist.json');
...
<section class="row">
  ...
  <div class="card">
    ...
    <?php else: ?>
      <div class="table-wrap">
      <table>
        <thead><tr><th>#</th><th>Name</th><th>Host:Port</th><th>Aktiv</th><th>Aktion</th></tr></thead>
        <tbody>
...
<section class="row">
    ...
    <?php else: ?>
      <div class="apps-grid">
        <?php foreach ($apps as $a):
          ...
          $avatar = $uuid ? 'https://minotar.net/avatar/'.htmlspecialchars($uuid).'/32' : '';
  ...
  <div class="card" style="flex:1;max-width:680px">
    ...
    <a href="admin_d.php"><button disabled class="btn">Spielerzahl bearbeiten</button></a><br>
    <a href="admin_messages.php" class="btn">Discord-Texte bearbeiten</a>
  </div>
</section>
